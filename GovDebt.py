...
class GovtDebt:
    ...
    issueYears = np.array([1,2,3,5,7,10,30])
    maxDuration = 30
    ...
    @staticmethod
    def calculateStaticDebt(Aggregate, Gtilde, Ctilde, Ttilde, debt_1, budget, T_model): 
        debts = np.append(debt_1, np.zeros(shape = (1,T_model-1)))
        deficits = Gtilde + Aggregate['bens'] + budget['infraSpending'] - Ttilde - Aggregate['revs'] - budget['lumpSumTaxes']
        for t in range(T_model-1):
            debts[t+1] = debts[t]*(1 + budget['debtrates'][t]) + deficits[t] - Ctilde[t] 
    ...
    #     Outputs: new debts, and 'pure' deficits (excluding Ctilde)
    @staticmethod
    def calculateDynamicDebts(Aggregate, Gtilde, Ctilde, Ttilde, debtPrev, T_model, budget, effectiveRatesByMaturity, debtDistribution, initRate):
        ...
        # rule)
        debts = np.append(debtPrev, np.zeros(shape = (1,T_model-1)))
        effRate = np.zeros(shape = (1,T_model))
        deficits = Gtilde + Aggregate['bens'] + budget['infraSpending'] 
        - Ttilde - Aggregate['revs'] - budget['lumpSumTaxes']
        ...
        # these aren't saved.
		# TBD: figure out what the point of this useless for-loop is
        for t in range(T_model):
            ...
            if t < T_model:
                (debts[t], _, effectiveRatesByMaturity[t], debtDistribution[t]) = (
                GovtDebt.GovtDebtDistribution(debts[t-1], debtDistribution[t-1], deficits[t-1] - Ctilde[t-1], effectiveRatesByMaturity[t-1], yieldCurve, budget[deflator[t]]/budget[deflator[t-1]], targetDistribution))
                if (t > 1):
                    effRate[t-1] = ((debts[t] - deficits[t-1] + Ctilde[t-1])/debts[t-1] - 1)
                else:
                    effRate[0] = initRate
            else: # setting last year effetive rate
                (tmpDebt, _, _, _) = (
                GovtDebt.GovtDebtDistribution(debts[t-1], debtDistribution[t-1], deficits[t-1] - Ctilde[t-1], effectiveRatesByMaturity[t-1], yieldCurve, budget[deflator[t-1]]/budget[deflator[t-2]], targetDistribution))
                if (t > 1):
                    effRate[t-1] = ((tmpDebt - deficits[t-1] + Ctilde[t-1])/debts[t-1] - 1)
                else:
                    effRate[0] = initRate              
    ...
    def calculateFixedDebts(closure_debttoout, deficits, outs, debt1, debtrates):
        ...
        new_debts = np.append(debt1, np.zeros(shape = (1,T)))
        outs = np.append(outs, outs[T])
        residual = np.zeros(shape = (1,T))
        for t in range(T):
    ...
    def GovtDebtDistribution(oldDebt, oldDebtDistribution, deficit, oldEffectiveRates, yieldCurve, inflation, targetDistribution):
        ...
        # less than one year.  
        newIssuesTotal = oldDebtLevels[1] + deficit + interestPaid
        
        # Advance the age of all old debt and add the new issues
        existingDebtLevels[0:(GovtDebt.maxDuration-1)] = oldDebtLevels[1:GovtDebt.maxDuration]
        existingDebtLevels[GovtDebt.maxDuration-1] = 0
        existingDebtRates[0:(GovtDebt.maxDuration-1)] = oldEffectiveRates[1:GovtDebt.maxDuration]
        existingDebtRates[GovtDebt.maxDuration-1] = 0
        ...
        newDebtRates = np.zeros(shape = (1,GovtDebt.maxDuration))
    ...
    def getSmallDistribution(debtDistribution):
        ...
        smallDistribution(max(GovtDebt['issueYears'].shape)) = 0
        for x in range(max(GovtDebt['issueYears'].shape)):
            smallDistribution[x] = np.sum(debtDistribution[(GovtDebt['issueYears'][max(x-1,1)]+1*(x>1)):GovtDebt['issueYears'][x]])
    ...
    def allocateNewDebt(newDebtDist, newIssuesTotal, targetDistribution):
        ...
        newIssuesDist[0:GovtDebt.maxDuration] = 0
        ...
        for x in range(max(shape(GovtDebt['issueYears']))):
