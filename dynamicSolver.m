...
methods (Static)    
    ...
    function [save_dir] = open(basedef, counterdef, partag)
        ...
        [save_dir] = dynamicSolver.transition('open', basedef, counterdef, partag);
    end
    ...
    function [save_dir] = closed(basedef, counterdef, partag)
        if ~exist('partag', 'var'), partag = ''; end
        [save_dir] = dynamicSolver.transition('closed', basedef, counterdef, partag);
    end
...
methods (Static, Access = private)
    ...
    function [save_dir] = transition(economy, basedef, counterdef, partag)
        ...
        switch economy
            case 'open'  , save_dir = dirFinder.open  (deep_params, plan, gcut);
            case 'closed', save_dir = dirFinder.closed(deep_params, plan, gcut);
        end
        ...
        if ~isbase
            ...
            switch economy
                case 'open'  , base_dir = dirFinder.open  (deep_params, 'base', +0.00);
                case 'closed', base_dir = dirFinder.closed(deep_params, 'base', +0.00);
            end
            ...
            if ~exist(base_aggregates, 'file')
                ...
                switch economy
                    case 'open'
                        uniquetag = sprintf('_open_plan=%s_gcut=%+0.2f'  , plan, gcut);
        ...
        if ~exist(ss_solution, 'file')
            ...
            switch economy
                case 'open'  , uniquetag = sprintf('_open_plan=%s_gcut=%+0.2f'  , plan, gcut);
                case 'closed', uniquetag = sprintf('_closed_plan=%s_gcut=%+0.2f', plan, gcut);
            end
        ...
        switch economy
            ...
            case 'closed'
                ...
                % Check for open economy dynamic aggregates and generate if not available
                open_aggregates = fullfile(open_dir, 'aggregates.mat');
                if ~exist(open_aggregates, 'file')
                    fprintf('\nOpen economy dynamic aggregates not found.  Calling open economy solver.\n')
                    
                    uniquetag = sprintf('_closed_plan=%s_gcut=%+0.2f', plan, gcut);
                    dynamicSolver.open( struct('beta', deep_params(1), 'gamma', deep_params(2), 'sigma', deep_params(3)), struct('plan', plan, 'gcut', gcut), uniquetag );
                    
                    % Check for open economy dynamic aggregates again, possibly generated by a parallel run
                    firstrun = false;
                    if ~exist(open_aggregates, 'file')
                        % Attempt to copy to permanent directory and continue if error encountered
                        % (Errors typically due to parallel copy attempt, which should result in at least one successful copy)
                        try copyfile([open_dir, uniquetag], open_dir), firstrun = true; catch, end
                    end
                    
                    fprintf('\nOpen economy dynamic aggregates generated')
                    if firstrun, fprintf('.'), else fprintf(' by a parallel run.'), end
                    
                    % Clean up temporary directory
                    rmdir([open_dir, uniquetag], 's')
        ...
        while ( eps > tol && iter < itermax )
            ...
            for idem = 1:ndem
                ...
                parfor i = 1:length(birthyears)
                    ...
                    % Generate distributions and cohort aggregates
