...
methods (Static, Access = private)
    ...
    function [save_dir] = transition(isopen, basedef, counterdef, partag)
        ...
        if ~isopen
            ...
            % Check for open economy aggregates and generate if not available
            open_aggregates = fullfile(open_dir, 'aggregates.mat');
            if ~exist(open_aggregates, 'file')
                fprintf('\nOpen economy aggregates not found.  Calling open economy solver.\n')
                ...
                % Check for open economy aggregates again, possibly generated by a parallel run
                firstrun = false;
                if ~exist(open_aggregates, 'file')
                    % Attempt to copy to permanent directory and continue if error encountered
                    % (Errors typically due to parallel copy attempt, which should result in at least one successful copy)
                ...
                fprintf('\nOpen economy aggregates generated')
                if firstrun, fprintf('.'), else fprintf(' by a parallel run.'), end
