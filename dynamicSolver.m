...
methods (Static, Access = private)
    ...
    function [save_dir] = transition(economy, basedef, counterdef, callertag)
        ...
        % Append caller tag to save directory name
        ...
        if ~isbase
            ...
            if ~exist(base_aggregates, 'file')
                ...
                dynamicSolver.transition(economy, basedef, [], callingtag );
                ...
                if ~exist(base_aggregates, 'file')
                    ...
                    try copyfile([base_dir, callingtag], base_dir), firstrun = true; catch, end
                end
        ...
        if ~exist(ss_solution, 'file')
            ...
            solve_ss([beta, gamma, sigma], false, callingtag);
            ...
            if ~exist(ss_solution, 'file')
                % Attempt to copy to permanent directory and continue if error encountered
                % (Errors typically due to parallel copy attempt, which should result in at least one successful copy)
                try copyfile([ss_dir, callingtag], ss_dir), firstrun = true; catch, end
            end
        ...
        switch economy
            ...
            case 'closed'
                ...
                if ~exist(open_aggregates, 'file')
                    ...
                    dynamicSolver.open( basedef, counterdef, callingtag );
                    ...
                    if ~exist(open_aggregates, 'file')
                        % Attempt to copy to permanent directory and continue if error encountered
                        % (Errors typically due to parallel copy attempt, which should result in at least one successful copy)
                        try copyfile([open_dir, callingtag], open_dir), firstrun = true; catch, end
                    end
