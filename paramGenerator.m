...
methods (Static)
    ...
    function s = grids(scenario)
        ...
        zage   = read_series('ConesaEtAl_WageAgeProfile.csv', [], ExecutionMode.sim_param());
    ...
    function s = demographics()
        param_dir = ExecutionMode.sim_param();
    ...
    function s = budget( scenario )
        ...
        cbo_param   = ExecutionMode.cbo_param();
        sim_param   = ExecutionMode.sim_param();
        ...
        taxplan_dir = ExecutionMode.taxplan_param();
        try
...
function [incomes, taxrates] = read_tax_rates( filename )
    ...
    filepath    = fullfile(ExecutionMode.taxplan_param(), filename);
