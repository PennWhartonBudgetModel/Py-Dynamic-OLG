...
matlab -nodesktop -nosplash -r "ExecutionMode.setToProduction(), mexBuilder.all(), modelProducer.define_runs($1)"
...
qsub -N run -t 1-$(ls -1 ./Runs | wc -l) \
     ...
     -b y 'matlab -nojvm -nosplash -r "ExecutionMode.setToProduction(), modelProducer.run(${SGE_TASK_ID})"'
...
qsub -N package_results -hold_jid run \
     ...
     -b y 'matlab -nodesktop -nosplash -r "ExecutionMode.setToProduction(), modelProducer.check_terminations(), modelProducer.package_results()"'
