...
class GenerateReports: 
    ...
    @staticmethod
    def MomentTable(scenario, parPoolNum, outputVar, varName1, list1, bound1, varName2, list2, bound2, outputYear, economyType):
        
        # This function generates a large table of conditional moments
        # Parameters:
        # scenario (the scenario being pulled)
        # parPoolNum: the number of parallel processors being used,
        # currently 1 to 3 (4 completely freezes the computer)
        ...
        #   list1 = [5,10,15,20] and set this to 0.  Then we'll get 5-10,
        #   10-15, 15-20 year olds.  
        ...
        # Delete and create the pools, minimum of 1 core, maximum of 3 (4
        # or more brings the computer to its knees and BSOD)

        # TBD figure out what this parallel pool thing is doing
        # delete(gcp('nocreate'))
        # parpool(min(max(1, parPoolNum),3))
        
        #Create the report structure.  
        report = {'year': outputYear,
                  'relation': economyType}
        report['outputNames'] = outputVar
        report['variable'] = outputVar
        
        # Filling in the components of the "report" structure that will be used
        # to determine the output
        
        report['firstCond'] = varName1
        report['secondCond'] = varName2
        ...
        firstLen = len(list1)
        secondLen = len(list2)
        outputTable = np.zeros(shape=(firstLen-firstOffset, secondLen-secondOffset))
        
        # Parallel outer loop.  
        # TBD figure out how to parallelize this for loop
        for indexOne in range(firstOffset, firstLen):
        
            # Need a separate report structure within the loop so it does not
            # conflict with the parallelization.
            report2 = report

            # Initializing the values of the row of the table we're filling in.
            tableRow = np.zeros(shape=(1,secondLen-secondOffset))
            ...
            for indexTwo in range(secondOffset, secondLen):
                ...
                valueOneBottom = list1[indexOne - firstOffset]
                ...
                valueTwoBottom = list2[indexTwo - secondOffset]
                ...
                report2['firstCondUpper'] = str(valueOne)
                report2['firstCondLower'] = str(valueOneBottom)
                report2['year'] = str(theYear)
                report2['secondCondUpper'] = str(valueTwo)
                report2['secondCondLower'] = str(valueTwoBottom)
                
                # Generating the actual value for this element of the table.
                # When createOutput returns a value, it's returned in a table
                # with the index in the first element, and the actual value in
                # the second element.  
                tmpTable = GenerateReports.createOutput(scenario, report2)
                
                # Printing out the index of the table element just computed (not necessary).
                print([indexOne, indexTwo])
                tableRow[0,indexTwo] = tmpTable[0,1]
                
            # Filling in the rows of the table
            outputTable[indexOne,:] = tableRow[0,:]
        ...
        # top-left value is "NaN".
        outputTable = np.vstack(list1[firstOffset:firstLen], outputTable)
        outputTable = np.vstack(np.hstack(None, list2[secondOffset:secondLen]), outputTable)
    ...
    def generateDistributions(outputDataStructure, dist, decisions, vars, scenario, T, variableList, experiments):
        ...
        ng     = s['ng']
        zs     = [sSteady['zs'], s['zs']]         # shocks grid (by demographic type and age)
        ...
        zLocGrid = np.array(range(nz))*np.ones(1,80)
        ...
        nossLocation = (b == min(min(b))) #TBD figure out what this line is and how to do it in python
        ...
        for economy in experiments:
            ...
            households[economy]['TOTINC'] = households[economy]['LABOR_INCOME'] + households[economy]['ASSETS'] * (np.ones(capsharesPM) - capsharesPM) * bondDividendRates + households[economy]['ASSETS'] * (capsharesPM) * equityrates
        ...
        if (vars['summary'] == 1):
            ...
            if vars['firstcond'] !='PRODUCTIVITY_INDEX' or vars['firstcond'] != 'AGE' or vars['firstcond'] != 'CITIZEN' or vars['firstcond'] != 'EARNINGS_SHOCK' or vars['firstcond'] != 'RETIRED' or vars['firstcond'] != 'WORKING' or vars['firstcond'] != 'LABOR':
                vars['firstcondtop'] =  vars['firstcondtop'] * scenario.modelunit_dollar
                vars['firstcondbottom'] =  vars['firstcondbottom'] * scenario.modelunit_dollar
            else:
                if vars['firstcond'] == 'AGE':
                    vars['firstcondtop'] = vars['firstcondtop'] - T['realage_entry'] + 1
                    vars['firstcondbottom'] =  vars['firstcondbottom'] - T['realage_entry'] + 1
            
            # Same thing for the second set of boundaries, if they are
            # being used.
            
            if 'secondcond' in vars.keys():    
                if vars['secondcond'] != 'PRODUCTIVITY_INDEX' or vars['secondcond'] != 'AGE' or vars['secondcond'] != 'CITIZEN' or vars['secondcond'] != 'EARNINGS_SHOCK' or vars['secondcond'] != 'RETIRED' or vars['secondcond'] != 'WORKING' or vars['secondcond'] != 'LABOR':
                   vars['secondcondtop'] =  vars['secondcondtop'] * scenario.modelunit_dollar
                   vars['secondcondbottom'] =  vars['secondcondbottom'] * scenario.modelunit_dollar
                else:      
                    if vars['secondcond'] == 'AGE':
                        vars['secondcondtop'] = vars['secondcondtop'] - T['realage_entry']
                        vars['secondcondbottom'] =  vars['secondcondbottom'] - T['realage_entry']
            
            theYear = vars['year'] - min(yearRange) + 1
            ...
            conditionalMatrix = np.zeros((households['base']['LABOR'][:,1]).shape)
            conditionalMatrix = conditionalMatrix + math.trunc(((households['base'][vars['firstcond']][:,theYear] <= vars['firstcondtop']) + (households['base'][vars['firstcond']][:,theYear] >= vars['firstcondbottom']))/2)
            if 'secondcond' in vars.keys():    
                conditionalMatrix = conditionalMatrix + math.trunc(((households['base'][vars['secondcond']][:,theYear] <= vars['secondcondtop']) + (households['base'][vars['secondcond']][:,theYear] >= vars['secondcondbottom']))/2)
            ...
            for numZ, numB, numAge, numG, numK in zip(iter(nz), iter(nb), iter(T_life), iter(ng), iter(nk)):
                ...
                if conditionalMatrix[currentStateIndexNum]:
                    ...
                    if vars['variable'] == 'EV':
                        ...
                        if vars['variable'] == 'AGE' or vars['variable'] == 'EARNINGS_SHOCK' or vars['variable'] == 'RETIRED' or vars['variable'] == 'WORKING' or vars['variable'] == 'LABOR':
                            sampleSummary[currentStateIndexNum] = households[economy][vars['variable']][currentStateIndexNum, theYear]
                        else:
                            sampleSummary[currentStateIndexNum] = households[economy][vars['variable']][currentStateIndexNum, theYear]/ scenario.modelunit_dollar
        ...
        for economy in experiments:
            ...
            tempRead = pd.read_table(file)
            tempRead.append([100, None, 1])     # Append last point for graph
            for q in range(T_model):
                outputDataStructure[economy]['percentileList']['data'][:,q] = tempRead[:,0]
                outputDataStructure[economy]['percentileList']['xaxis'][:,q] = tempRead[:,0]
                outputDataStructure[economy]['a_distdata_thresh']['data'][:,q] = tempRead[:,1]
                outputDataStructure[economy]['a_distdata_thresh']['xaxis'][:,q] = tempRead[:,1]
                outputDataStructure[economy]['a_distdata_cumul']['data'][:,q] = tempRead[:,2]      
                outputDataStructure[economy]['a_distdata_cumul']['xaxis'][:,q] = tempRead[:,0]  
            ...
            tempRead.append([100, None, 1])    # Append last point for graph
            for q in range(T_model):
                outputDataStructure[economy]['l_distdata_thresh']['data'][:,q] = tempRead[:,1]
                outputDataStructure[economy]['l_distdata_thresh']['xaxis'][:,q] = tempRead[:,0]
                outputDataStructure[economy]['l_distdata_cumul']['data'][:,q] = tempRead[:,2] 
                outputDataStructure[economy]['l_distdata_cumul']['xaxis'][:,q] = tempRead[:,0]
            ...
            if 'capDensity' in variableList:
                for t in range(T_model):
                    ...
                    for q in range(nk):
                        capIndicator = (k == kv[q])
                        capDensity[q] = np.sum(distrib[economy][:,t] * capIndicator[:,t])  # * ageIndicator[:,t] * capIndicator[:,t]
            ...
            if 'consDensity' in variableList:
                ...
                retDist = retDist / (np.ones(numObs, 1) * np.sum(retDist, 1))
                for t in range(T_model):
            ...
            if 'c_lorenz' in variableList:  
                ...
                retDist = distrib[economy] * retiredLocation
                numObs = max(retDist.shape)
                retDist = retDist / (np.ones(numObs, 1) * np.sum(retDist, axis=1))
            ...
            if 'thresh' in variableList or 'cumul' in variableList:  
                for q in range(max(outputDataStructure[economy]['percentileList']['data'][:,1].shape)):
                ...
                for q in range(T_model):
                    outputDataStructure[economy]['a_distmodel_cumul']['data'][:,q] = outputDataStructure[economy]['a_distmodel_cumul']['data'][:,q] / outputDataStructure[economy]['a_distmodel_cumul']['data'][max(outputDataStructure[economy]['percentileList']['data'][:,1].shape),q]
                    outputDataStructure[economy]['l_distmodel_cumul']['data'][:,q] = outputDataStructure[economy]['l_distmodel_cumul']['data'][:,q] / outputDataStructure[economy]['l_distmodel_cumul']['data'][max(outputDataStructure[economy]['percentileList']['data'][:,1].shape),q]
    ...
    def createTable(outputDataStructure, T, vars, scale, graph):
        ...
        yearRange = T['TransitionFirstYear']-T['TransitionLastYear']-1
    ...
    def parseVars(reportTemplate, scenario, steadyState):
        ...
        tmp_var = [s.strip() for s in reportTemplate['variable'].split(',')] 
        tmp_rel = [s.strip() for s in reportTemplate['relation'].split(',')]
        ...
        if 'firstCond' in reportTemplate.keys():
            ...
            tmp_firstCond = [s.strip() for s in reportTemplate['firstCond'].split(',')]
            tmp_firstCondTop = [s.strip() for s in reportTemplate['firstCondUpper'].split(',')]
            tmp_firstCondBottom = [s.strip() for s in reportTemplate['firstCondLower'].split(',')]
            
            assert max(tmp_var.shape) == 1, 'Only one summary statistic is allowed.'
            assert max(tmp_firstCondTop.shape) == 1, 'Wrong number of top values for the first conditional.'
            assert max(tmp_firstCondBottom.shape) == 1, 'Wrong number of bottom values for the first conditional.'
            assert 1 == max(tmp_rel.shape), 'The number of variables does not match the number of relations.'
            ...
            if 'secondCond' in reportTemplate:
                tmp_secondCond = [s.strip() for s in reportTemplate['secondCond'].split(',')]
                ...
                tmp_secondCondTop = [s.strip() for s in reportTemplate['secondCondUpper'].split(',')]
                tmp_secondCondBottom = [s.strip() for s in reportTemplate['secondCondLower'].split(',')]
                assert max(tmp_secondCondTop.shape) == 1, 'Wrong number of top values for the second conditional.'
                assert max(tmp_secondCondBottom.shape) == 1, 'Wrong number of bottom values for the second conditional.'
        ...
        tmp_out = [s.strip() for s in reportTemplate['outputNames'].split(',')]
        tmp_typ = [s.strip() for s in reportTemplate['type'].split(',')]
       
        # These are the acceptable values for each field in the output
        # structure.
        types = ['level', 'per_diff', 'lev_diff', 'delta']
        relations = ['c_b', 'c_s', 's_b', 'b', 's', 'c']
        filetypes = ['xls', 'csv']
        graphaxes = ['l','r']
        dollar = ['y','n']
        threshold = dollar
        relationsIfYear = ['b', 's', 'c']
        ...
        assert max(tmp_var.shape) == max(tmp_out.shape), 'The number of variables does not match the number of output names.'
        assert max(tmp_var.shape) == max(tmp_typ.shape), 'The number of variables does not match the number of types.'
        assert max(tmp_var.shape) == max(tmp_rel.shape), 'The number of variables does not match the number of relations.'
        ...
        if (validDist > 0):
            tmp_firstVar = [s.strip() for s in reportTemplate['firstVar'].split(',')]
            tmp_secondVar = [s.strip() for s in reportTemplate['secondVar'].split(',')]
            tmp_pt = [s.strip() for s in reportTemplate['percentTop'].split(',')]
            tmp_pb = [s.strip() for s in reportTemplate['percentBottom'].split(',')]
            tmp_thresh = [s.strip() for s in reportTemplate['threshold'].split(',')]
            
            assert max(tmp_var.shape) == max(tmp_firstVar.shape), 'The number of variables does not match the number of first variables for densities.'
            assert max(tmp_var.shape) == max(tmp_secondVar.shape), 'The number of variables does not match the number of second variables for densities.'
            assert max(tmp_var.shape) == max(tmp_pt.shape), 'The number of variables does not match the number of percentiles at the top.'
            assert max(tmp_var.shape) == max(tmp_pb.shape), 'The number of variables does not match the number of percentiles at the bottom.'
            assert max(tmp_var.shape) == max(tmp_thresh.shape), 'The number of variables does not match the number of thresholds.'
       
        
        # This loads cohorts, if necessary   
        if validCohort:
            tmp_cohort = [s.strip() for s in reportTemplate['cohort'].split(',')] 
            assert max(tmp_var.shape) == max(tmp_cohort.shape), 'The number of variables does not match the number of cohorts.'
            checkNum = []
            for q in range(max(tmp_cohort.shape)):
        ...
        if validGraphs > 0:
            tmp_axis = [s.strip() for s in reportTemplate['axis'].split(',')]
            assert max(tmp_var.shape) == max(tmp_axis.shape), 'The number of variables does not match the number of graphs.'
            graphs = 1
        ...
        if validYear:
            tmp_year = [s.strip() for s in reportTemplate['year'].split(',')]
            assert max(tmp_var.shape) == max(tmp_year.shape), 'The number of variables does not match the number of years.'
        
        # This loads in the contructed variables, assuming we have a scale factor 
        
        if (validScale > 0):
            tmp_scale = [s.strip() for s in reportTemplate['scaleFactor'].split(',')]
            tmp_dollar = [s.strip() for s in reportTemplate['dollar'].split(',')]
            assert max(tmp_var.shape) == max(tmp_scale.shape), 'The number of variables does not match the number of scale values.'
            assert max(tmp_var.shape) == max(tmp_dollar.shape), 'The number of variables does not match the number of dollar scale values.'
        ...
        for p in range(max(tmp_var.shape)):
            ...
            assert not ((tmp_typ[p] == 'level') and (max(tmp_rel[p].shape)>1)), 'Level output is not compatible with the type of relation.'
            if validYear:
            ...
            if validSummary > 0:   
                vars[p]['summary'] = 1
                assert (max(tmp_var.shape) == 1), 'Must be one variable for a summary statistic.'
                vars[p]['firstcond'] = tmp_firstCond
                vars[p]['firstcondtop'] = float(tmp_firstCondTop)
                vars[p]['firstcondbottom'] = float(tmp_firstCondBottom)
                ...
                if 'secondCond' in reportTemplate.keys():
                    vars[p]['secondcond'] = tmp_secondCond
                    tmp_secondCond = [s.strip() for s in reportTemplate['secondCond'].split(',')]
                    vars[p]['secondcondtop'] = float(tmp_secondCondTop)
                    vars[p]['secondcondbottom'] = float(tmp_secondCondBottom)
    ...
    # the first value for the aggregates and markets
        ...
        yearRange = range(T['TransitionFirstYear'] - 1, T['TransitionLastYear'])
        ...
        for p in experiments:
            ...
            for o in range(len(fieldNames)):  # copying the aggregate values
                ...
                if fieldNames[o, 1] in outputDataStructure[p]:
        ...
        for p in experiments:
            ...
            for o in range(max(fieldNames.shape)):  # copying the aggregate values
    ...
    def gini(distrib, x, T_model):
        ...
        for t in range(T_model):
           ...
           ginicoeff[t] = 1 - np.sum((lorenz_var[0:-2,t] + lorenz_var[1:-1,t]) * np.diff(lorenzPop[:,t]))
