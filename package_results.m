                ...
                for labor_elas = [0.25, 0.50, 0.75, 1.00]
                    for savings_elas = [0.25, 0.50, 0.75, 1.00]
                        ...
                        if (openness == 0) || (openness == 1)
                            ...
                            % Identify working directories
                            [~, save_dir_base_open]     = identify_dirs('open',  beta, gamma, sigma, 'base');
                            if (openness == 1)
                                [~, save_dir_base_case] = identify_dirs('open',  beta, gamma, sigma, 'base');
                                [~, save_dir_dynamic]   = identify_dirs('open',  beta, gamma, sigma, plan{1});
                                [~, save_dir_static]    = identify_dirs('open',  beta, gamma, sigma, plan{1});
                            else
                                [~, save_dir_dynamic] = identify_dirs('closed', beta, gamma, sigma, plan{1}, gcut);
                                [~, save_dir_static]  = identify_dirs('closed', beta, gamma, sigma, plan{1}, gcut);
                                if use_dyn_base == 0
                                    [~, save_dir_base_case]  = identify_dirs('open'  , beta, gamma, sigma, 'base');
                                elseif use_dyn_base == 1
                                    [~, save_dir_base_case]  = identify_dirs('closed', beta, gamma, sigma, 'base', gcut);
                                end
                            end
