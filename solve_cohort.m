...
assert( isa(contax_lumps     , 'double' ) && (size(contax_lumps     , 1) <= T_max ) && (size(contax_lumps     , 2) <= 1 ) );
assert( isa(contax_rates     , 'double' ) && (size(contax_rates     , 1) <= T_max ) && (size(contax_rates     , 2) <= 1 ) );
...
for t = T_active:-1:1
    ...
    % Consumption taxes for this year
    con_lump            = contax_lumps          (year);
    con_rate            = contax_rates          (year);
...
function [v] ...
    ...
    consumption = (resources - s) * (1 - con_rate) - con_lump;
