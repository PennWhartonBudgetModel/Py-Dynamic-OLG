...
methods (Static)
    ...
    function [] = package_results(batchID)
        ...
        results_dir = env.csv(scenarios(1));  % We don't have a good place for this, shouldn't be on Scenario
        timestamp   = datestr(now, 'yyyy-mm-dd HH:MM');
        fid         = fopen(fullfile(results_dir, 'dynamicModelTag.txt'), 'w');
        fprintf( fid, 'TimeStamp=%s\n', timestamp );
        fprintf( fid, 'ShortDescription=Generated by dynamic model version %s.\n', env.get_commit_tag() );
        fprintf( fid, 'LongDescription=\n');
        fclose(fid);
    ...
    function [] = export_results(irun)
        ...
        if usedynamicbaseline
            ...
            Dynamic_open_base   = load(fullfile(env.save(base_scenario.open())  , 'dynamics.mat'));
            Dynamic_closed_base = load(fullfile(env.save(base_scenario.closed()), 'dynamics.mat'));
        end
        ...
        for j = 1:size(codes,1)
            ...
            csvfile = fullfile(csv_dir, sprintf('%u-%u.csv', scenario.ID, codes{j,1}));
