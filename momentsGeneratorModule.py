...
class MomentsGenerator:
    ...
    # Table - assets and labor income thresholds (real 2018 USD) for all percentiles
    def thresholdTable(self):
        
        threshold_table = pd.DataFrame({'Percentile': self.a_distdata['percentile'],
                              'a_data': self.a_distdata['threshold2018dollars'],
                              'a_model': self.a_distmodel['threshold']/self.scenario['modelunit_dollar'],
                              'l_model': self.l_distdata['threshold2018dollars'],
                              'l_data': (self.l_distmodel['threshold']/self.scenario['modelunit_dollar'])})
        return threshold_table
    
    # Table - total income thresholds (model only - 2018 USD) for all percentiles
    def totincTable(self):
            
        totinc_table = pd.DataFrame({'Percentile': self.a_distdata['percentile'],
                         'total_income': self.t_distmodel['threshold']/self.scenario['modelunit_dollar'],
                         'cumShare': self.t_distmodel['cumulativeShare']})
    ...
    def plot_a_threshold(self):
        ...
        wealth_lobound = np.maximum(self.a_distdata['threshold2018dollars']/1000, 0)
        ...
        plt.plot(self.a_distdata['percentile'],wealth_lobound, linewidth = 2, label = 'data    (gini = 0.857)')
    ...
    def plot_l_threshold(self):
        ...
        labinc_lobound = np.maximum(self.l_distdata['threshold2018dollars']/1000, 0)
        ...
        plt.plot(self.l_distdata['percentile'],labinc_lobound, linewidth = 2, label = 'data    (gini = 0.4858)')
    ...
    def plot_a_dist(self):
        ...
        kv     = self.kv
        kvstr  = np.array(map(str, np.around(kv/self.scenario['modelunit_dollar'])))
        nk     = self.DIST.shape[1]
        kdist  = np.zeros(nk)
        for ik in range(nk):
        ...
        plt.xlim(1,nk)
        ticks = range(1,nk+1,2)
        plt.xticks(ticks, kvstr)
        plt.ylabel('share of population', fontsize = 13)
            
        
    # Table - Distribution of individuals by asset holdings grid points
    def a_distTable(self):
        ...
        a_dist_table = pd.DataFrame({'Grid_value': kvstr,
                                     'share_population': kdist})
...
def gini(dist,x,makeplot=False):
...
# rightsum. We can put all of this into one line.
    ...
    ginicoeff = 1 - sum((relz[0:-1]+relz[1:]) * np.diff(relpop))
